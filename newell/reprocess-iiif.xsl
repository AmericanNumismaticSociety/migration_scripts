...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://www.tei-c.org/ns/1.0" xmlns:tei="http://www.tei-c.org/ns/1.0"
	...
	<xsl:template match="tei:surface">
		...
		<xsl:variable name="ratio"></xsl:variable>
		...
		<xsl:element name="surface" namespace="http://www.tei-c.org/ns/1.0">
			...
			<xsl:choose>
				<xsl:when test="$height &gt;= $width">
					...
					<xsl:attribute name="ulx" select="ceiling(($width div 2) + (($width div 2) * @ulx))"></xsl:attribute>
