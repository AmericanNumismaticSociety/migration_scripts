        ...
        while (false !== ($file = readdir($handle))) {
            ...
            if (strpos($file, '.xml') !== FALSE){
                ...
                if (($readFile = fopen($fileName, 'r')) === FALSE){
                    error_log($recordId . ' failed to open temporary file (accnum likely broken) at ' . date(DATE_W3C) . "\n", 3, "/var/log/numishare/error.log");
                } else {
                    ...
                    //set curl opts
                    curl_setopt($putToExist,CURLOPT_URL, $eXist_url . $collection . '/objects/' . $recordId . '.xml');
                    ...
                    if (curl_error($putToExist) === FALSE){
                        error_log($recordId . ' failed to upload to eXist at ' . date(DATE_W3C) . "\n", 3, "/var/log/numishare/error.log");
                    } else {
                        if ($http_code == '201'){
                            $datetime = date(DATE_W3C);
                            echo "Writing {$recordId}.\n";
                            error_log("{$recordId}: {$datetime}\n", 3, "/var/log/numishare/success.log");                            
                        }
                    }
                    //close eXist curl
